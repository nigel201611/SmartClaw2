...
export function registerAuthIPCHandlers(mainWindow: BrowserWindow) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      rememberMe: boolean
    ): Promise<AuthIPCResponse<any>> => {
      try {
        const result = await authManager.login(username, password, homeserverUrl);
  ...
  ipcMain.handle(
    ...
    async (
      ...
      password: string,
      homeserverUrl: string
    ): Promise<AuthIPCResponse<void>> => {
      try {
        const result = await authManager.register(username, password, homeserverUrl);
  ...
  ipcMain.handle(
    ...
    async (): Promise<AuthIPCResponse<void>> => {
      try {
  ...
  // 恢复会话
  ipcMain.handle(
    AUTH_IPC_CHANNELS.RESTORE_SESSION,
    async (): Promise<AuthIPCResponse<any>> => {
      try {
  ...
  // 获取认证状态
  ipcMain.handle(
    AUTH_IPC_CHANNELS.GET_STATUS,
    async (): Promise<AuthIPCResponse<string>> => {
      try {
  ...
  // 检查是否已认证
  ipcMain.handle(
    AUTH_IPC_CHANNELS.IS_AUTHENTICATED,
    async (): Promise<AuthIPCResponse<boolean>> => {
      try {
  ...
  // 保存凭证
  ipcMain.handle(
    AUTH_IPC_CHANNELS.SAVE_CREDENTIALS,
    async (event, credentials: Credentials): Promise<AuthIPCResponse<void>> => {
      try {
  ...
  // 获取凭证
  ipcMain.handle(
    AUTH_IPC_CHANNELS.GET_CREDENTIALS,
    async (): Promise<AuthIPCResponse<Credentials | null>> => {
      try {
  ...
  // 清除凭证
  ipcMain.handle(
    AUTH_IPC_CHANNELS.CLEAR_CREDENTIALS,
    async (): Promise<AuthIPCResponse<void>> => {
      try {
