...
export function registerMatrixIPCHandlers(mainWindow: BrowserWindow) {
  ...
  // 登出
  // ipcMain.handle(MATRIX_IPC_CHANNELS.LOGOUT, async (): Promise<MatrixIPCResponse<void>> => {
  //   try {
  //     await client.logout();
  ...
  // 获取房间列表
  // ipcMain.handle(MATRIX_IPC_CHANNELS.GET_ROOMS, async (): Promise<MatrixIPCResponse<RoomInfo[]>> => {
  //   try {
  ...
  // 发送消息
  // ipcMain.handle(MATRIX_IPC_CHANNELS.SEND_MESSAGE, async (event, roomId: string, text: string): Promise<MatrixIPCResponse<string>> => {
  //   try {
